...
static int
dmapi_summary(char *buffer, char **start, linux_off_t offset,
...
static int __init dmapi_init(void)
{
	int ret;
...
static void __exit dmapi_uninit(void)
{
	if( dm_uninit() == 0 ){
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("DMAPI for XFS");
