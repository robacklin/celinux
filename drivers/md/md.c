...
/*
 * validate_sb
 * This checks for obvious inconsistencies that may have occurred
 * with values in the raid superblock.  Further work needs to be
 * done to find the root cause of each of these, but this at least
 * keeps the superblock sane, so the system will boot.
 * Currently this is called as a safeguard before a write of the sb.
 * Along with any future work, the mddev->private values (e.g. raid1)
 * need to stay sync'd up with the superblock.     -- Andy Cress
 */
int validate_sb(mdp_super_t *sb, int op)
{
   int changed = 0;
#ifdef TEST
   printk("md%d:validate_sb(%d_x): working=%d spare=%d failed=%d nr_disks=%d\n",            sb->md_minor, op, sb->working_disks, sb->spare_disks,
...
#endif  
   if (changed) {
     printk("md%d:validate_sb(%d): working=%d spare=%d failed=%d nr_disks=%d\n",            sb->md_minor, op, sb->working_disks, sb->spare_disks,
            sb->failed_disks, sb->nr_disks);
...
static int write_disk_sb(mdk_rdev_t * rdev)
{
	...
	if (!sync_page_io(dev, sb_offset<<1, MD_SB_BYTES, rdev->sb_page,
								WRITE)) {
...
repeat:
	...
	ITERATE_RDEV(mddev,rdev,tmp) {
		...
		printk("%s ", partition_name(rdev->dev));
		/* check for rdev->sb NULL here also.  ARC*/
		if (!rdev->faulty && !rdev->alias_device && rdev->sb) {
			printk("[events: %08lx]",
				(unsigned long)rdev->sb->events_lo);
			err += write_disk_sb(rdev);
		} else
			printk(")\n");
		if (i > MD_SB_DISKS) {  /* ARC*/
			printk("md_update_sb: iterate_rdev limit reached\n");
			// MD_BUG();  /*%%%%*/
			break; 
			}
		i++;
