...
#if 1 /* linux-pm */
		assert_constraints(bus, drv->constraints);
#endif /* linux-pm */
	}
	return error;
}
...
static int scale_error;

static int do_driver_scale(struct device_driver *drv, void *data)
{
	scale_error |= driver_scale(drv, data);
	return scale_error;
}
...
int bus_scale(struct bus_type * bus, struct bus_op_point * op)
{
	...
	if (bus->bus_op) {
		*bus->bus_op = *op;
		bus_for_each_drv(bus, NULL, (void *) DPM_SCALE_NOTIFY,
				 do_driver_scale);
		...
		if (! scale_error) {
			...
			bus_for_each_drv(bus, NULL, (void *) DPM_SCALE,
					 do_driver_scale);
		}
	}
			       ...
			       struct constraints *drv_constraints)
{
	...
	if (! drv_constraints || ! bus->constraints)
		return 0;

	get_bus(bus);
	
	for (i = 0; i < drv_constraints->count; i++) {
		for (j = 0; j < bus->constraints->count; j++) {
			if (bus->constraints->param[j].id == 
			    drv_constraints->param[i].id) {

				if (drv_constraints->param[i].min > 
				    bus->constraints->param[j].min) {
					bus->constraints->param[j].min =
						drv_constraints->param[i].min;
					bus_constrained = 1;
				}

				if (drv_constraints->param[i].max < 
				    bus->constraints->param[j].max) {
					bus->constraints->param[j].max =
						drv_constraints->param[i].max;
					bus_constrained = 1;
				}
			...
			struct constraints *drv_constraints)
{
	...
	drv_constraints->asserted = 1;

	if (assert_a_constraint(bus, drv_constraints))
...
static int do_assert_a_constraint(struct device_driver * drv, void * data)
{
	if (drv->constraints && drv->constraints->asserted)
		(void) assert_a_constraint(drv->bus, drv->constraints);
...
void bus_reeval_constraints(struct bus_type * bus)
{
	...
	bus_for_each_drv(bus, NULL, NULL, do_assert_a_constraint);
