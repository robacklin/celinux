...
#define NE_IRQ (-1)
#endif
static int ne_ioa = NE_IOA;
static int ne_irq = NE_IRQ;
MODULE_PARM(ne_ioa,"i");
MODULE_PARM(ne_irq,"i");
MODULE_PARM_DESC(ne_ioa,"NE I/O address");
MODULE_PARM_DESC(ne_irq,"NE irq number");
...
retry:
	...
	if (ei_status.word16) {
              outsw(NE_BASE + NE_DATAPORT, (unsigned char*)buf, count>>1);
	} else {
              outsb(NE_BASE + NE_DATAPORT, (unsigned char*)buf, count);
	}
...
static int __init ne_eth_setup( char *str )
{
      int ints[3];
