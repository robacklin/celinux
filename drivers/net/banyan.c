...
banyan_tx(struct net_device *dev)
{
	...
	while (lp->tx_count && IS_DMA_USED(td->cmdstat)) {
		...
		if ((ltd.devcs & 0xffffffc0) == 0) {
			...
			dbg(0, "%s: no devcs status\n", __func__);
			dbg(0, "%s:    dptr=%08x, td=%08x\n", __func__,
			    readl(&lp->tx_dma_regs->dmadptr),
			    (u32)kseg1_to_phys(td));
			dbg(0, "%s:    cmdstat=%08x, ca=%08x, "
			    "devcs=%08x, link=%08x\n", __func__,
			    ltd.cmdstat, ltd.curr_addr, ltd.devcs, ltd.link);
			dbg(0, "%s:    nI=%d, nO=%d, cnt=%d\n", __func__,
			    lp->tx_next_in, lp->tx_next_out, lp->tx_count);
			lp->stats.tx_errors++;
			...
			lp->stats.tx_errors++;
			/* should never happen */
			dbg(1, "%s: split tx ignored\n", __func__);
		} else if (IS_TX_TOK(ltd.devcs)) {
			/* transmit OK */
			lp->stats.tx_packets++;
		} else {
			dbg(0, "%s: error, devcs=0x%08x\n",
			    __func__, ltd.devcs);
...
static int banyan_init(struct net_device *dev)
{
	...
	/* loop back */
	lp->rd_ring[BANYAN_NUM_RDS-1].link = kseg1_to_phys(&lp->rd_ring[0]);
