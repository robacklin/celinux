 ...
 * gt96100eth.c
 *
 * Driver for builtin ethernet on the GT96100 MIPS
 * communications controller companion chip.
 *
...
dump_tx_desc(int dbg_lvl, struct net_device *dev, int i)
{
	...
	dbg(dbg_lvl, "Tx descriptor at %p:\n", td);
	dbg(dbg_lvl,
...
dump_rx_desc(int dbg_lvl, struct net_device *dev, int i)
{
	...
	dbg(dbg_lvl, "Rx descriptor at %p:\n", rd);
...
gt96100_add_hash_entry(struct net_device *dev, unsigned char* addr)
{
	...
	dump_hw_addr(3, dev, "%s: nib swap/invt addr=", __func__, hash_ea);
...
enable_ether_irq(struct net_device *dev)
{
	...
	u32 intMask;
	/*
	 * route ethernet interrupt to GT_INT0 for port 0,
	 * GT_INT1 for port 1.
	 */
	int intr_mask_reg = (gp->port_num == 0) ?
		GT96100_INT0_HIGH_MASK : GT96100_INT1_HIGH_MASK;
...
int gt96100_init_module(void)
{
	...
	if (cpuConfig & (1<<12)) {
		printk(KERN_ERR __FILE__ ": must be in Big Endian mode!\n");
		return -ENODEV;
	}
...
gt96100_probe1(int port_num)
{
	...
	/* request_region is failing for some reason, so disabled for now */
#if 0
	...
	gtif->dev = dev;
	gp = dev->priv;
	memset(gp, 0, sizeof(*gp)); // clear our private area
	...
	if ((retval = parse_mac_addr(dev, gtif->mac_str))) {
		printk(KERN_ERR "%s: MAC address parse failed\n", __func__);
		retval = -EINVAL;
		goto free_netdev;
	}
	...
	/* finally, register the network device */
 ...
 free_netdev:
	...
	kfree (dev);
	printk(KERN_ERR "%s failed.  Returns %d\n", __func__, retval);
	return retval;
}
...
gt96100_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	gp->tx_next_in = (nextIn + 1) & (TX_RING_SIZE - 1);
...
gt96100_rx(struct net_device *dev, u32 status)
{
	...
	for (nextOut = gp->rx_next_out; nextOut != cdp;
	     nextOut = (nextOut + 1) & (RX_RING_SIZE - 1)) {
		...
		if (cmdstat & (u32)rxOwn) {
			//err(__func__ ": device owns descriptor!\n");
...
gt96100_tx_complete(struct net_device *dev, u32 status)
{
	...
	for (nextOut = gp->tx_next_out; nextOut != cdp;
	     nextOut = (nextOut + 1) & (TX_RING_SIZE - 1)) {
	...
	*/
	if (dev->mc_count) {
		...
		for (mcptr = dev->mc_list; mcptr; mcptr = mcptr->next) {
			dump_hw_addr(2, dev, __func__ ": addr=",
				     mcptr->dmi_addr);
