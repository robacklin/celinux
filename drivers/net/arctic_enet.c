...
static int arctic_enet_suspend(struct device *dev, u32 state, u32 level);
static int arctic_enet_resume(struct device *dev, u32 level);

static struct device_driver arctic_enet_driver_ldm = {
       name:	  "RTL8019", 
       devclass:  NULL,
       probe:     NULL,
       suspend:   arctic_enet_suspend,
       resume:    arctic_enet_resume,
       remove:    NULL,
       constraints: NULL,
};

static struct device arctic_enet_device_ldm = {
       name: "Arctic2 Ethernet",
       bus_id: "arctic_enet",
       driver: NULL,
       power_state: DPM_POWER_ON,
};

static void arctic_enet_ldm_driver_register(void)
{
   extern void ebc_driver_register(struct device_driver *driver);

   ebc_driver_register(&arctic_enet_driver_ldm);
}

static void arctic_enet_ldm_device_register(void)
{
   extern void ebc_device_register(struct device *device);

   ebc_device_register(&arctic_enet_device_ldm);
}
static void arctic_enet_ldm_driver_unregister(void)
{
   extern void ebc_driver_unregister(struct device_driver *driver);

   ebc_driver_unregister(&arctic_enet_driver_ldm);
}

static void arctic_enet_ldm_device_unregister(void)
{
   extern void ebc_device_unregister(struct device *device);

   ebc_device_unregister(&arctic_enet_device_ldm);
}

static int arctic_enet_suspend(struct device *dev, u32 state, u32 level)
{
	struct net_device *netdev = &arctic_enet_dev;
	...
	case SUSPEND_POWER_DOWN:
		...
		/* TO-DO - Is it necessary to save state??? 
		 Shouldn't we wait for transmitted packets to complete??? */
...
static int arctic_enet_resume(struct device *dev, u32 level)
{
	...
	struct net_device *netdev = &arctic_enet_dev;
	struct ei_device *ei_local = (struct ei_device *) netdev->priv;
	...
	case RESUME_POWER_ON:
                ...
                /* When resuming the device we assume that it has been powered
                   off, and go through a complete reinitialization.  This is
                   done with interrupts disabled to aviod problems from devices
                   that may not have been shut down. */

		disable_irq_nosync(netdev->irq);
		spin_lock(&ei_local->page_lock);

		/* Set register page 0 */
		temp = E8390_PAGE0 + E8390_NODMA + E8390_STOP;
		writeb(temp, iobase8 + NE_CMD);

		/* Switch to register page 3 */
 		writeb(temp | 0xc0, iobase8 + NE_CMD);

		/* Set EEM1=EEM0=1 in 9346CR */ 
		temp2 = readb(iobase8 + 0x01); 
		writeb(temp2 | 0xc0, iobase8 + 0x01); 

		/* Clear sleep and powerdown bits (2:1) in CONFIG3 */
		writeb(readb(iobase8 + 0x06) & ~0x06, iobase8 + 0x06); 

		/* Return to previous mode */ 
		writeb(temp2, iobase8 + 0x01);
		
		/* Return to register page 0 */
		writeb(temp, iobase8 + NE_CMD);

		/* Reset and reinit again */ 
		arctic_enet_reset_8390(netdev); 	

		/* We only reinitialize the state if the network interface is open
		   and running, otherwise, NS8390_init() will be called if the
		   interface is opened (again). */

		if (netif_running(netdev)) {
			NS8390_init(netdev, 1);        
		}
...
#ifdef CONFIG_ARCTIC2 /* linux-pm */
        arctic_enet_ldm_driver_register(); 
        arctic_enet_ldm_device_register();
#endif        
...
#ifdef CONFIG_ARCTIC2 /* linux-pm */
        arctic_enet_ldm_driver_unregister();
        arctic_enet_ldm_device_unregister(); 
#endif    
