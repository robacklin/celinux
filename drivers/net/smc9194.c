...
*/
#if	defined(__sh__) || \
	defined(CONFIG_OMAP_1510P1) || \
	defined(CONFIG_OMAP_INNOVATOR)
#undef USE_32_BIT
#else
#define USE_32_BIT 1
#endif
	...
	!defined(CONFIG_ASSABET_NEPONSET) && \
...
/* This enables an interrupt in the interrupt mask register.
 * Some boards (such as the OMAP Innovator) are
 * wired in such a way that both 8-bit and 16-bit accesses to 
 * the chip are interpreted by the chip as 16-bit accesses.  
 * In order for these boards to work we must avoid 8-bit 
 * writes to avoid corrupting the other half of a 16-bit register.
 * In this case we write a zero to the interrupt acknowledge 
 * register, which is harmless.
 */
#define SMC_ENABLE_INT(x)				\
	{						\
		u16 mask;				\
		mask = smc_inw(ioaddr, INTERRUPT) & 0xff00; \
		mask |= (x) << 8;			\
		smc_outw(mask, ioaddr, INTERRUPT);	\
	}
...
/* Acknowledge an interrupt, preserving the interrupt
 * mask register.
 */
#define SMC_ACK_INT(x)					\
	{						\
		u16 ack;				\
		ack = smc_inw(ioaddr, INTERRUPT) & 0xff00; \
		ack |= (x) & 0xff;			\
		smc_outw(ack, ioaddr, INTERRUPT);	\
	}
...
DECLARE_WAIT_QUEUE_HEAD(susp_wq);
static int susp_lockout = 0;
static int susp_ignoretimeout = 0;
...
static void smc_setmulticast(struct net_device *dev, int count, struct dev_mc_list * addrs)
{
	...
	for (i = 0; i < 8 ; i += 2) {
		smc_outw(multicast_table[i] | (multicast_table[i + 1] << 8), 
			ioaddr, MULTICAST1 + i);
	}
}
...
int __init smc_init(struct net_device *dev)
{
	...
	if (machine_is_pfs168()) {
		volatile u_int * ioaddr;
		u_long flags;

		/* First, enable access to the SMC91C96 via some undocumented
		 * black magic register bit flipping.
		*/
		ioaddr = (volatile u_int *)ioremap(0x1a020000, 8*1024);
		if (!ioaddr)
			return -ENOMEM;
		((u_char *)(ioaddr))[0] = 0x41;
		udelay(100);
		((u_char *)(ioaddr))[8] = 0x00;
		udelay(100);
		iounmap(ioaddr); /* don't need this anymore... */

		/*
		 * The PFS-168 uses GPIO26 for the SMC9196 interrupts.
		 * So set up GPIO26 for edge triggered interrupts now.
		 */
		...
		ioaddr = (volatile u_int *)ioremap(0x18000000, 8*1024);
		if (!ioaddr)
			return -ENOMEM;

		dev->irq = IRQ_GPIO26;

		ret = smc_probe(dev, (int)ioaddr);
		if (ret)
			iounmap(ioaddr);
	}

#elif	defined(CONFIG_OMAP_1510P1) || \
	defined(CONFIG_OMAP_INNOVATOR)

	int base_addr = OMAP1510P1_FPGA_ETHR_BASE;

	SET_MODULE_OWNER(dev);

	dev->irq = INT_ETHER;

	/* Take the ethernet chip out of reset and wait for the
	   EEPROM load to complete
	*/
        outb(inb(OMAP1510P1_FPGA_RST) & ~1, OMAP1510P1_FPGA_RST);
        udelay(750);

	if (smc_probe(dev, base_addr) == 0) {
		smc9194_ldm_register(dev); /* MVL-CEE */
		return 0;
	}
...
static int __init smc_probe(struct net_device *dev, int ioaddr)
{
	...
	*/
	...
	if ((dev->dev_addr[0] & 0x01) != 0) {
		/*
		 * Assigned address is a multicast MAC which is not good.
...
#if 1 /* linux-pm */
	wait_event(susp_wq, susp_lockout == 0);
#endif /* linux-pm */
...
static void smc_timeout(struct net_device *dev)
{
	...
	if (susp_ignoretimeout) {
		printk("%s_timeout: Ignoring timeout due to suspend.\n",
		       dev->name);
		susp_ignoretimeout = 0;
		netif_wake_queue(dev);
		return;
	}
#endif /* linux-pm */
...
#if 1 /* linux-pm */
	if (susp_lockout) {
		/*  tell the card to get rid of this packet */
