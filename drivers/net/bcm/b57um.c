...
char bcm5700_version[] = "4.0.4";
char bcm5700_date[] = "(10/25/02)";
...
#endif
void b57_free_remaining_rx_bufs(UM_DEVICE_BLOCK *pUmDevice);
void b57_validate_param_range(struct net_device *dev, int *param,
					...
					int board_idx)
{
	...
	*dev_out = NULL;
	...
	b57_validate_param_range(dev, &mtu[board_idx], "mtu", 1500, 9000, 1500);
...
//	tasklet_disable(&pUmDevice->tasklet);
	...
	b57_free_remaining_rx_bufs(pUmDevice);
	bcm5700_freemem(dev);
...
STATIC int bcm5700_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCNICE:
	{
		...
		else if( nrq->cmd == NICE_CMD_DIAG_SUSPEND ) {
			bcm5700_intr_off(pUmDevice);
		...
		else if( nrq->cmd == NICE_CMD_DIAG_RESUME ) {
			if (pUmDevice->suspended) {
...
MM_GetConfig(PLM_DEVICE_BLOCK pDevice)
{
	...
	b57_validate_param_range(dev, &auto_speed[index], "auto_speed",
		0, 1, 1);
	...
	if (line_speed[index] == 0) {
		...
		b57_validate_param_range(dev, &full_duplex[index],
			"full_duplex", 0, 1, 1);
	...
	b57_validate_param_range(dev, &rx_flow_control[index],
		"rx_flow_control", 0, 1, 0);
	if (rx_flow_control[index] != 0) {
		pDevice->FlowControlCap |= LM_FLOW_CONTROL_RECEIVE_PAUSE;
	}
	b57_validate_param_range(dev, &tx_flow_control[index],
		"tx_flow_control", 0, 1, 0);
	if (tx_flow_control[index] != 0) {
		pDevice->FlowControlCap |= LM_FLOW_CONTROL_TRANSMIT_PAUSE;
	}
	b57_validate_param_range(dev, &auto_flow_control[index],
	...
	if (auto_flow_control[index] != 0) {
		if (pDevice->DisableAutoNeg == FALSE) {
			...
			printk(KERN_WARNING "%s: Conflicting auto_flow_control parameter (%d), using 0\n",
				dev->name, auto_flow_control[index]);
		}
	...
	b57_validate_param_range(dev, &tx_pkt_desc_cnt[index],
		"tx_pkt_desc_cnt", 1, MAX_TX_PACKET_DESC_COUNT, TX_DESC_CNT);
	pDevice->TxPacketDescCnt = tx_pkt_desc_cnt[index];
	b57_validate_param_range(dev, &rx_std_desc_cnt[index],
	...
	if (mtu[index] <= 1514) {
		...
		b57_validate_param_range(dev, &rx_jumbo_desc_cnt[index],
...
#ifdef BCM_INT_COAL
	b57_validate_param_range(dev, &adaptive_coalesce[index],
		"adaptive_coalesce", 0, 1, 1);
	pUmDevice->adaptive_coalesce = adaptive_coalesce[index];
	if (!pUmDevice->adaptive_coalesce) {
		b57_validate_param_range(dev, &rx_coalesce_ticks[index],
		...
		b57_validate_param_range(dev, &rx_max_coalesce_frames[index],
		...
		b57_validate_param_range(dev, &tx_coalesce_ticks[index],
		...
		b57_validate_param_range(dev, &tx_max_coalesce_frames[index],
		...
		b57_validate_param_range(dev, &stats_coalesce_ticks[index],
...
#ifdef BCM_WOL
	b57_validate_param_range(dev, &enable_wol[index], "enable_wol",
		0, 1, 0);
	if (enable_wol[index]) {
		pDevice->WakeUpModeCap = LM_WAKE_UP_MODE_MAGIC_PACKET;
		pDevice->WakeUpMode = LM_WAKE_UP_MODE_MAGIC_PACKET;
...
#endif
	b57_validate_param_range(dev, &scatter_gather[index], "scatter_gather",
		0, 1, 1);
	b57_validate_param_range(dev, &tx_checksum[index], "tx_checksum",
		0, 1, 1);
	b57_validate_param_range(dev, &rx_checksum[index], "rx_checksum",
		0, 1, 1);
	if (pDevice->ChipRevId == T3_CHIP_ID_5700_B0) {
...
void
b57_free_remaining_rx_bufs(UM_DEVICE_BLOCK *pUmDevice)
{
	UM_PACKET *pUmPacket;
	struct sk_buff *skb;
	int cnt, i;
...
void
b57_validate_param_range(struct net_device *dev, int *param, char *param_name,
	int min, int max, int deflt)
{
...
struct net_device *
b57_find_peer(struct net_device *dev)
{
