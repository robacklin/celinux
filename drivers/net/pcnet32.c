...
#define	KZP_MAC_ADDRESS	0x800fffb0	// KZP-BIOS-MAC-ADDRESS 
#endif
		...
		struct pci_dev *pdev)
{
...
#ifdef CONFIG_KZP01
    pcnet32_dwio_write_csr(ioaddr, 0,0);	/* dwio mode */
    pcnet32_dwio_write_bcr(ioaddr,20,2);	/* 32bit mode */
#endif
...
#if CONFIG_KZP01
#ifdef SWSTYLE3
	if((a->read_bcr(ioaddr,25) & 0xff)==0){	/* RAM size check OK */
	    a->write_bcr(ioaddr,25,0x17);	/* default ram size 12k byte */
	    a->write_bcr(ioaddr,26,0x04);	/* sram_bnd = 4 */
	}
#else
     ...
     */
#ifdef CONFIG_KZP01
    if(*(unsigned char *)KZP_MAC_ADDRESS != 0x01){
	panic("pcnet32.c: MAC address not set\n");
    }else{
	dev->dev_addr[0] = *(unsigned char *)(KZP_MAC_ADDRESS+1);
	dev->dev_addr[1] = *(unsigned char *)(KZP_MAC_ADDRESS+2);
	dev->dev_addr[2] = *(unsigned char *)(KZP_MAC_ADDRESS+3);
	dev->dev_addr[3] = *(unsigned char *)(KZP_MAC_ADDRESS+4);
	dev->dev_addr[4] = *(unsigned char *)(KZP_MAC_ADDRESS+5);
	dev->dev_addr[5] = *(unsigned char *)(KZP_MAC_ADDRESS+6);
    }
#else
    for (i = 0; i < 3; i++) {
	unsigned int val;
...
#ifdef CONFIG_KZP01
    if ((0x2623<=chip_version) && (chip_version<=0x2625)) { /* Version 0x2623-0x2625 */
#else
...
pcnet32_open(struct net_device *dev)
{
    ...
    /* switch pcnet32 to 32bit mode */
#ifdef SWSTYLE3
    lp->a.write_bcr (ioaddr, 20, 3);
    lp->a.write_bcr (ioaddr, 18, lp->a.read_bcr (ioaddr,18)|0x860);
#else
...
#ifdef CONFIG_KZP01
		    if ((status & 0x1800) && (status & 0x0040))
#else
		    if (status & 0x1800)
#endif
			lp->stats.collisions++;
...
#ifdef CONFIG_KZP01
		dma_cache_inv((void *)skb->data,pkt_len);
#endif
		skb->dev = dev;
		if (!rx_in_place) {
