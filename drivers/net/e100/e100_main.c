...
char e100_driver_version[]="2.2.21";
...
static void e100_initiate_hwi(struct e100_private *);
...
e100_found1(struct pci_dev *pcid, const struct pci_device_id *ent)
{
	...
	bdp->mdix_status = "Unknown";
	bdp->cable_status = "Unknown";
	...
	if (netif_carrier_ok(dev)) {
		...
		if (bdp->params.e100_speed_duplex != E100_AUTONEG) {
			...
			e100_force_speed_duplex_to_phy(bdp);
				...
				/* Initiate cable diagnostic */
				e100_initiate_hwi(bdp);
		}
	}
...
int 
e100_notify_netdev(struct notifier_block *nb, unsigned long event, void *p)
{
	struct e100_private *bdp;
...
static void
e100_initiate_hwi(struct e100_private *bdp)
{	
	...
	mod_timer(&(bdp->hwi_timer), jiffies + (HZ / 1000) + 1);
}
...
e100_do_hwi(struct net_device *dev)
{
	...
	if (bdp->saved_same == MAX_SAME_RESULTS) {
		if ((open_circut && !(short_circut)) ||
			...
			if (open_circut) {
				if (near_end) 
					bdp->cable_status = "Open Circuit Near End";
				else 
					bdp->cable_status = "Open Circuit Far End";
			} else {
				if (near_end) 
					bdp->cable_status = "Short Circuit Near End";
				else 
					bdp->cable_status = "Short Circuit Far End";
			}
			goto done;
		}
	}
