/*
 *  linux/drivers/video/s3c2440fb.c
 *
 ...
 *  11/19/2003 : Hayeong Kim <hayeong.kim@samsung.com>
 *  	support LDM
 *
...
void (*s3c2440fb_blank_helper)(int blank);
EXPORT_SYMBOL(s3c2440fb_blank_helper);

static struct s3c2440fb_rgb	rgb_8 = {
...
static struct s3c2440fb_rgb	def_rgb_16 = {
...
static struct s3c2440fb_rgb	xxx_tft_rgb_16 = {
    	red:	{offset: 11,	length: 5, },
	green:	{offset: 5,	length: 6, },
	blue:	{offset: 0,	length: 5, },
	transp:	{offset: 0,	length: 0, },
...
static struct s3c2440fb_mach_info * __init
s3c2440fb_get_machine_info(struct s3c2440fb_info *fbi)
{
    struct s3c2440fb_mach_info *inf = NULL;
...
{
    struct s3c2440fb_info *fbi = (struct s3c2440fb_info *)info;
...
static int
s3c2440fb_validate_var(struct fb_var_screeninfo *var,
...
static int
s3c2440fb_setpalettereg(u_int regno, u_int red, u_int green, u_int blue,
...
static int
s3c2440fb_setcolreg(u_int regno, u_int red, u_int green, u_int blue,
		    u_int trans, struct fb_info *info)
{
    struct s3c2440fb_info *fbi = (struct s3c2440fb_info *)info;
	...
	case FB_VISUAL_PSEUDOCOLOR:
	    ret = s3c2440fb_setpalettereg(regno, red, green, blue, trans, info);
	    break;
    }
...
static int s3c2440fb_activate_var(struct fb_var_screeninfo *var, struct s3c2440fb_info *fbi)
{
    struct s3c2440fb_lcd_reg new_regs;
...
static inline void s3c2440fb_set_truecolor(u_int is_true_color)
{
    if (is_true_color) {
    }
    else {
...
static void
s3c2440fb_hw_set_var(struct fb_var_screeninfo *var, struct s3c2440fb_info *fbi)
{
    ...
    fb_set_cmap(&fbi->fb.cmap, 1, s3c2440fb_setcolreg, &fbi->fb);

    s3c2440fb_set_truecolor(var->bits_per_pixel >= 16);

    s3c2440fb_activate_var(var, fbi);
...
static int
s3c2440fb_set_var(struct fb_var_screeninfo *var, int con, struct fb_info *info)
{
    struct s3c2440fb_info *fbi = (struct s3c2440fb_info *)info;
    ...
    err = s3c2440fb_validate_var(var, fbi);
	      ...
	      struct fb_info *info)
{
	...
	err = fb_set_cmap(cmap, kspc, s3c2440fb_setcolreg, info);
...
static int
s3c2440fb_get_fix(struct fb_fix_screeninfo *fix, int con, struct fb_info *info)
{
...
static int
s3c2440fb_get_var(struct fb_var_screeninfo *var, int con, struct fb_info *info)
{
...
static int
s3c2440fb_get_cmap(struct fb_cmap *cmap, int kspc, int con, struct fb_info *info)
{
    struct fb_cmap *dcmap = get_con_cmap(info, con);
...
#ifdef CONFIG_PM
static int
s3c2440fb_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
...
static int s3c2440fb_suspend(struct device *dev, u32 state, u32 level);
static int s3c2440fb_resume(struct device *dev, u32 level);

static struct device s3c2440fb_device = {
	name:		"S3C2440 LCD controller",
	bus_id:		"lcd",
	driver:		NULL,
};

static struct device_driver s3c2440fb_driver = {
	name:		"s3c2440fb",
	devclass:	NULL,
	probe:		NULL,
#ifdef CONFIG_PM
	suspend: 	s3c2440fb_suspend,
	resume:		s3c2440fb_resume,
#else
	suspend: 	NULL,
	resume:		NULL,
#endif
...
static void s3c2440fb_ldm_register(void)
{
	extern void ahb_bus_register(struct device *device, struct device_driver *driver);

	ahb_bus_register(&s3c2440fb_device, &s3c2440fb_driver);
	return;
}

/* #ifdef CONFIG_PM? CONFIG_LDM? */
static int 
s3c2440fb_suspend(struct device *dev, u32 state, u32 level)
{
}
...
static int s3c2440fb_switch(int con, struct fb_info *info)
{
    struct s3c2440fb_info *fbi = (struct s3c2440fb_info *)info;
    struct display *disp;
    struct fb_cmap *cmap;
    ...
    s3c2440fb_set_var(&fbi->fb.var, con, info);
 ...
 */
static int s3c2440_pm_callback(struct pm_dev *pm_dev, pm_request_t req, void *data)
{
	struct s3c2440fb_info *fbi = pm_dev->data;
	u_long flags;
...
static int __init s3c2440fb_map_video_memory(struct s3c2440fb_info *fbi)
{
...
static int s3c2440fb_updatevar(int con, struct fb_info *info)
{
    return 0;
}

static void s3c2440fb_blank(int blank, struct fb_info *info)
{
}
...
{
    struct s3c2440fb_mach_info *inf;
    struct s3c2440fb_info *fbi;

    fbi = kmalloc(sizeof(struct s3c2440fb_info) + sizeof(struct display) +
	    	  sizeof(u16)*16, GFP_KERNEL);

    if (!fbi)
	return NULL;

    memset(fbi, 0, sizeof(struct s3c2440fb_info) + sizeof(struct display));

    fbi->currcon		= -1;

    strcpy(fbi->fb.fix.id, S3C2440_NAME);
    ...
    strcpy(fbi->fb.modename, S3C2440_NAME);
    strcpy(fbi->fb.fontname, "Acorn8x8");

    fbi->fb.fbops		= &s3c2440fb_ops;
    fbi->fb.changevar		= NULL;
    fbi->fb.switch_con		= s3c2440fb_switch;
    fbi->fb.updatevar		= s3c2440fb_updatevar;
    fbi->fb.blank		= s3c2440fb_blank;
    ...
    inf	= s3c2440fb_get_machine_info(fbi);
...
{
    GPCCON = 0xaaaaaaaa;   /*VD[0]~VD[7]*/
    GPDCON = 0xaaaaaaaa;   /* VD[8]~VD[24]*/
...
#ifdef CONFIG_LDM 
static inline void s3c2440_lcd_enable(void)
{
	//LCDCON1 |= LCD1_ENVID;
}

static inline void s3c2440_lcd_disable(void)
{

}
#endif

int __init s3c2440fb_init(void)
{
    struct s3c2440fb_info *fbi;
    int ret;

#ifdef CONFIG_LDM
    s3c2440fb_ldm_register();
#endif
    ...
    ret = s3c2440fb_map_video_memory(fbi);
    if (ret)
	goto failed;
   ...
   /* enable the LCD controller) */
	printk("Installed S3C2440 frame buffer\n");
...
int __init s3c2440fb_setup(char *options)
{
    return 0;
}
