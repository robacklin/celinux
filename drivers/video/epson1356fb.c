 ...
 * fbaddr is at least aligned on a half-word boundary.
 */
static inline void
		...
		blt_info_t* blt)
{
	...
	writew(cpu_to_le16(width-1), &bltreg->width0);
	writew(cpu_to_le16(height-1), &bltreg->height0);

	// program color of fill
	writew(cpu_to_le16(blt->fg_color), &bltreg->fg_color0);
...
e1356_cfbX_cursor(struct display *p, int mode, int x, int y) 
{
	...
	if ((x != info->cursor.x) || (y != info->cursor.y) ||
		...
		writew(cpu_to_le16(x), &inkcurs->x_pos0);
		writew(cpu_to_le16(y), &inkcurs->y_pos0);
		...
		struct fb_info_e1356* info)
{
	...
	writew(cpu_to_le16(height), &dispcfg->vdh0);
	...
	writew(cpu_to_le16(addr_offset), &dispmode->mem_addr_offset0);
...
static u8
calc_mclk(struct fb_info_e1356 * info, int* mclk_src)
{
	struct e1356fb_fix * epfix = &info->fix;
	int i, mclk=0, max_mclk;
	int clksrc[3];
	u8 mclk_cfg=0;
	
	max_mclk = (info->chip == 7) ? 50000 : 40000;
	
	clksrc[0] = epfix->clki;
	clksrc[1] = epfix->busclk;
	clksrc[2] = (info->chip == 7) ? epfix->clki3 : 0;
	
	// Find the highest allowable MCLK
	for (i=0; i<3; i++) {
		if (clksrc[i] <= max_mclk && clksrc[i] > mclk) {
			mclk = clksrc[i];
			mclk_cfg = (u8)i;
		}
		if (clksrc[i]/2 <= max_mclk && clksrc[i]/2 > mclk) {
			mclk = clksrc[i]/2;
			mclk_cfg = (u8)i | 0x10;
		}
	}
	...
	epfix->mclk = mclk;
	*mclk_src = (mclk_cfg & 0x10) ? mclk*2 : mclk;
...
e1356fb_init(void)
{
	...
	fb_info.chip = rev_code >> 2;
	fb_info.chip_rev = rev_code & 0x03;

	if (fb_info.chip == 0x04) {
		printk("Found S1D13506 Display Controller\n");
	}
	else if (fb_info.chip == 0x07) {
		printk("Found S1D13806 Display Controller\n");
	}
	else {
	...
	case DISP_TYPE_LCD:
		fb_info.max_pixclock = 40000;
		break;
	case DISP_TYPE_TFT:
	case DISP_TYPE_CRT:
		fb_info.max_pixclock = (fb_info.chip == 7) ? 65000 : 40000;
		break;
	case DISP_TYPE_NTSC:
	case DISP_TYPE_PAL:
	...
	mclk_cfg = calc_mclk(&fb_info, &mclk_src);
	...
	printk("e1356fb: clocks (kHz): busclk=%d mclk=%d clki=%d "
	       "clki2=%d clki3=%d\n",
	       epfix->busclk, epfix->mclk, epfix->clki,
	       epfix->clki2, epfix->clki3);

	periodMCLK = 1000000L / epfix->mclk;   // in nano-seconds
	periodBCLK = 1000000L / epfix->busclk; // in nano-seconds
	if (((fb_info.chip == 4) &&
	     (readb(&reg->md_cfg->md_cfg_stat1) & (1<<4))) ||
	    ((fb_info.chip == 7) &&
	     (readb(&reg->md_cfg->md_cfg_stat0) & (1<<5)))) {
		printk("e1356fb: BCLK = busclk/2\n");
		periodBCLK *= 2;
	}
	
	if ((epfix->system == SYS_PB1000) || (epfix->system == SYS_PB1500))
		writeb(0x00, &reg->clk_cfg->cpu2mem_wait_sel);
	else if (periodMCLK > periodBCLK + 4)
		writeb(0x02, &reg->clk_cfg->cpu2mem_wait_sel);
	else if (2*periodMCLK > periodBCLK + 4)
	...
	while ((mclk_src >> (6 + rr_div)) < epfix->mem_refresh)
		if (--rr_div < 0) {
			printk("e1356fb: can't set refresh rate: %d KHz\n",
			       epfix->mem_refresh);
			printk("e1356fb: mclk_src = %d KHz\n", mclk_src);
			goto ret_enxio;
		}
	...
	if (fb_info.chip == 7) /* 806 */
		epfix->mem_smr = 0;
	else if (epfix->mem_smr < MEM_SMR_CBR ||
		 epfix->mem_smr > MEM_SMR_NONE) {
	...
	writew(cpu_to_le16(dram_timing), &reg->mem_cfg->dram_timings_ctrl0);
...
e1356fb_setup(char *options, int *ints)
{
	...
	for(this_opt=strtok(options, ","); this_opt;
		...
		if (!strncmp(this_opt, "noaccel", 7)) {
		...
		} else if (!strncmp(this_opt, "clki3:", 6)) {
			boot_fix.clki3 = simple_strtoul(this_opt+6, NULL, 0);
			...
			} else if  (!strncmp(this_opt+7, "superh", 7)) {
				boot_fix = systems[SYS_SUPERH].fix;
				boot_par = systems[SYS_SUPERH].par;
			}
		} else {
