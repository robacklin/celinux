...
static struct chips_init_reg chips_init_cr[] = {
	{ 0x11, 0x0d },		// write enable CR0-CR7 (bit7=0)
	{ 0x00, 0x7f },
	{ 0x01, 0x63 },
	{ 0x02, 0x63 },
	{ 0x03, 0x83 },
...
//**** MCLK PLL
	...
	{ 0xcc, 200 },		/* memory clock ratio PLL-M */
	{ 0xcd, 14 },		// PLL-N FVCO = 14.3*M/N = 204MHz
	{ 0xce, 0x90 },		// PLL-PD divide2 

//********** DCLK1 PLL
	{0xc4, 0x84 },			// PLL-M	43KHz/68Hz
	{0xc5, 40 },			// PLL-N	SVGA 43K/68Hz
//	{0xc5, 22 },			// PLL-N	XGA 58K/71Hz
	{0xc7, 0x20 },			// PLL-PD


//	{ 0xcf, 0x06 },		// CLK-config (default)
//	{ 0xd0, 0x0e },		// Powerdown ctrl
	{ 0x09, 0x01 },
//	{ 0x0a, 0x02 },
	{ 0x0a, 0x03 },		// Frame buffer = Linear & page enable

	{ 0x0b, 0x01 },		// PCI burst write enable
//	{ 0x20, 0x00 },		// BitBLT config
	{ 0x40, 0x03 },		// memory acc ctrl
	{ 0x80, 0x82 },		// 8bitDAC over_screen_color
#if 1	/* collor 8bit */
	{ 0x81, 0x12 },		// 8bpp
	{ 0x82, 0x08 },		// pix cfg2 GR-palett enable
#else	/* collor 15bit */
	{ 0x81, 0x14 },		// 16bpp  5-5-5 RGB
	{ 0x82, 0x00 },		// pix cfg2 GR-palett disable
#endif
//	{ 0xa0, 0x00 },
//	{ 0xa8, 0x00 },
};
...
static void __init chips_hw_init(struct fb_info_chips *p)
{
	...
	outb( 0xFF, 0x3C6 );		/* PALMASK */
	outb( 0x20, 0x3C0 );		// video enable

	chipsfb_blank( 0, &p->info );
}
...
static void __init init_chips(struct fb_info_chips *p)
{
	...
	strcpy(p->fix.id, "C&T 69000");
	p->fix.smem_start = p->frame_buffer_phys;

// FIXME: Assumes 1MB frame buffer, but 65550/69000 supports 1MB or 2MB.
...
// -- Tim Seufert
	...
	printk("fb%d: Chips 69000 frame buffer (%dK RAM detected)\n",
	...
	if (!console_fb_info) {
		...
		strncpy(display_info.name, "chips69000",
...
int __init chips69000_init(void)
{
	struct pci_dev *dp = NULL;

#define PCI_DEVICE_ID_CT_68000 0x00C0
	while ((dp = pci_find_device(PCI_VENDOR_ID_CT,
				     PCI_DEVICE_ID_CT_68000, dp)) != NULL)
...
#ifdef CONFIG_ARM_KZP01
	p->frame_buffer_phys  = addr + 0x80000000;
	p->frame_buffer = addr + KZP01_PCI_MEM_WIN;
#endif
#ifdef CONFIG_MIPS_KZP01
	p->frame_buffer_phys  = (p->frame_buffer = addr + KZP01_PCI_MEM_WIN)-0xa0000000;
#endif
#ifdef CONFIG_SH_KZP01
	p->frame_buffer = addr;
	p->frame_buffer_phys = KZP01_PCI_MEM_WIN + (addr & 0x00ffffff);
	kzp_set_fb_bank( addr );
#endif
