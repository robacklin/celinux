...
#ifdef CONFIG_PM
#include <linux/pm.h>
static struct pm_dev *pxa_ac97_pm_dev = NULL;
struct ac97_codec *ac_pm_use_point;
#endif
...
{
	struct ac97_codec *codec =( struct ac97_codec *)(dev->data);
	int ret;
	...
	if (rqst == PM_RESUME ) {
		LUB_HEXLED = 0x10097;
		printk(KERN_INFO "In file %s, pm_callback called rqst == PM_RESUME.\n", __FILE__ );
		CKEN |= CKEN2_AC97;
		set_GPIO_mode(GPIO31_SYNC_AC97_MD);
		set_GPIO_mode(GPIO30_SDATA_OUT_AC97_MD);
		set_GPIO_mode(GPIO28_BITCLK_AC97_MD);
		...
		if (ret != 1)
			printk(KERN_INFO "After resume AC97 codec probe fail.\n");
		// need little hack for UCB1400 (should be moved elsewhere)
	...
	if (rqst == PM_SUSPEND) {
	/*
	 *  Shutdown ACLINK and Disable clock should be delayed
	 *  until other device, such as  ts, etc shutdown.
	 *  Clock is also disabled in pm.c
	 */
		LUB_HEXLED = 0x97;
		if (!pxa_ac97_refcount) {
			GCR = GCR_ACLINK_OFF;
...
#ifdef CONFIG_PM
	ac_pm_use_point = dummy;
	pxa_ac97_pm_dev = pm_register(PM_SYS_DEV, PM_SYS_UNKNOWN, pxa_ac97_pm_callback);
	printk(KERN_INFO "In file %s, pm_registered.\n", __FILE__);
	if (pxa_ac97_pm_dev == NULL)
		printk("PXA AC97: unable to register in PM.\n");
	else    {
		pxa_ac97_pm_dev->data = dummy;
	}
#endif
	return 0;
}
