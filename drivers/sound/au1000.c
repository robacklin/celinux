...
static void dac_dma_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (buff_done != (DMA_D0 | DMA_D1)) {
		...
		newptr = dac->dmaaddr + dac->nextOut + dac->dma_fragsize;
...
static void adc_dma_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (buff_done != (DMA_D0 | DMA_D1)) {
		...
		newptr = adc->dmaaddr + adc->nextIn + adc->dma_fragsize;
...
{
	char * bufptr =
		to_user ? db->rawbuf + db->nextOut : db->rawbuf + db->nextIn;
                        ...
                        unsigned int cmd, unsigned long arg)
{
	...
	case SNDCTL_DSP_GETIPTR:
		...
		if (!s->dma_adc.stopped) {
			...
			count += diff;
			cinfo.bytes += diff;
			cinfo.ptr =  s->dma_adc.nextIn + diff;
		} else
	...
	case SNDCTL_DSP_GETOPTR:
		...
		if (!s->dma_dac.stopped) {
			...
			count -= diff;
			cinfo.bytes += diff;
			cinfo.ptr = s->dma_dac.nextOut + diff;
		} else
