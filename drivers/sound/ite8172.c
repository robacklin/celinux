 ...
 *    07.26.2003  MIPS virt_to_phys/bus/page no longer accepts KSEG1 virtual
 *                pointers as input - fixes for this. [stevel]
 */
...
struct it8172_state {
	...
	struct dmabuf {
		...
		unsigned numfrag;
		unsigned fragshift;
		int nextIn;  // byte index
		int nextOut; // byte index
		int count;
		int curBufPtr;
...
static inline struct page * kseg1_to_page(void * kseg1_addr)
{
	return virt_to_page(KSEG0ADDR(kseg1_addr));
}
...
static inline void dealloc_dmabuf(struct it8172_state *s, struct dmabuf *db)
{
	...
	if (db->rawbuf) {
		...
		pend = kseg1_to_page(db->rawbuf +
				     (PAGE_SIZE << db->buforder) - 1);
		for (page = kseg1_to_page(db->rawbuf); page <= pend; page++)
...
static void it8172_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (isc & ISC_CCI) {
		if (adc->count > adc->dmasize - adc->fragsize) {
			...
			newptr = adc->dmaaddr + adc->nextIn + 2*adc->fragsize;
	...
	if (isc & ISC_PCI) {
		newptr = dac->dmaaddr + dac->nextOut + 2*dac->fragsize;
...
{
	char* bufptr = to_user ?
		db->rawbuf + db->nextOut : db->rawbuf + db->nextIn;
...
static int it8172_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (remap_page_range(vma->vm_start, db->dmaaddr,
