...
#ifdef CONFIG_SERIAL_TXX9_CONSOLE
#define TXX9_CONFIG_EXPR port - &rs_ports[0] != sercons.index
#else
#define TXX9_CONFIG_EXPR 1
#endif
	if (TXX9_CONFIG_EXPR) {
		local_irq_save(flags);
		/*
		...
		sio_out(port, TXX9_SILCR,
			...
			((port->flags & TXX9_SERIAL_USE_SCLK) ?
				   ...
				   int baud_base, int have_cts)
{
	...
	/* negative baud_base value means SCLK */
	if (baud_base < 0) {
		rs_ports[line].baud_base = -baud_base;
		rs_ports[line].flags |= TXX9_SERIAL_USE_SCLK;
		//printk(KERN_INFO "TXX9_SERIAL_USE_SCLK is enabled\n");
	} else {
		rs_ports[line].baud_base = baud_base;
		//printk(KERN_INFO "TXX9_SERIAL_USE_SCLK is disabled, baud_base is %x\n",baud_base);
	}
...
static __init int serial_console_setup(struct console *co, char *options)
{
	...
	if (port->flags & TXX9_SERIAL_USE_SCLK)
		sio_out(port, TXX9_SILCR, cval | TXX9_SILCR_SCS_SCLK_BG);
	else
		sio_out(port, TXX9_SILCR, cval | TXX9_SILCR_SCS_IMCLK_BG);
	sio_out(port, TXX9_SIBGR, quot | TXX9_SIBGR_BCLK_T0);
	/* no RTS/CTS control */
