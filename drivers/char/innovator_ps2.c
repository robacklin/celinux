...
SS(int value)
{
	innovator_fpga_hid_frob(FPGA_HID_nSS, value ? FPGA_HID_nSS : 0);
}

static void
SCLK(int value)
{
	innovator_fpga_hid_frob(FPGA_HID_SCLK, value ? FPGA_HID_SCLK : 0);
}

static void
MOSI(int value)
{
	innovator_fpga_hid_frob(FPGA_HID_MOSI, value ? FPGA_HID_MOSI : 0);
}

static u8
MISO(void)
{
	return ((innovator_fpga_hid_rd() & FPGA_HID_MISO) ? 1 : 0);
}

static int
ATN(void)
{
	return ((innovator_fpga_hid_rd() & FPGA_HID_ATN) ? 1 : 0);
}
...
{
	innovator_fpga_hid_wr(FPGA_HID_SCLK | FPGA_HID_MOSI);
	mdelay(1);
	innovator_fpga_hid_set_bits(FPGA_HID_RESETn);
}
...
static int innovator_ps2_scale(struct bus_op_point *op, u32 level);

static int innovator_ps2_pid;
static DECLARE_COMPLETION(kusbdpmd);
static int omap1510_usbpmd_req;
static wait_queue_head_t kusbdpmd_wait;

static struct device_driver innovator_ps2_driver_ldm = {
	.name		= PFX,
	.probe		= NULL,
	.suspend	= innovator_ps2_suspend,
	.resume		= innovator_ps2_resume,
	.scale		= innovator_ps2_scale,
	.remove		= NULL,
	.constraints	= NULL,
};

static struct device innovator_ps2_device_ldm = {
	.name		= "Innovator/OMAP1510 PS/2",
	.bus_id		= "PS/2",
	.driver		= &innovator_ps2_driver_ldm,
	.power_state	= DPM_POWER_ON,
};

static void
innovator_ps2_ldm_driver_register(void)
{
	extern void mpu_public_driver_register(struct device_driver *driver);
	mpu_public_driver_register(&innovator_ps2_driver_ldm);
}

static void
innovator_ps2_ldm_device_register(void)
{
	extern void mpu_public_device_register(struct device *device);
	mpu_public_device_register(&innovator_ps2_device_ldm);
}

static void
innovator_ps2_ldm_driver_unregister(void)
{
	extern void mpu_public_driver_unregister(struct device_driver *driver);
	mpu_public_driver_unregister(&innovator_ps2_driver_ldm);
}

static void
innovator_ps2_ldm_device_unregister(void)
{
	extern void mpu_public_device_unregister(struct device *device);
	mpu_public_device_unregister(&innovator_ps2_device_ldm);
}

static int
innovator_ps2_scale(struct bus_op_point *op, u32 level)
{
	/* REVISIT */
	return 0;
}
...
static int
innovator_ps2_suspend(struct device *dev, u32 state, u32 level)
{
	u8 pmcomm;
...
static int
innovator_ps2_resume(struct device *dev, u32 level)
{
	u8 pmcomm;
...
handle_print_scr(int up)
{
	if (up) {
		...
		handle_scancode(0xb7, 1);
		handle_scancode(0xe0, 1);
		handle_scancode(0xaa, 1);
	} else {
		handle_scancode(0xe0, 0);
		handle_scancode(0x2a, 0);
		handle_scancode(0xe0, 0);
		handle_scancode(0x37, 0);
	}
}
...
handle_pause(void)
{
	handle_scancode(0xe1, 0);
	handle_scancode(0x1d, 0);
	handle_scancode(0x45, 0);
	handle_scancode(0xe1, 0);
	handle_scancode(0x9d, 0);
	handle_scancode(0xc5, 0);
}
...
process_keyboard_report(struct innovator_hid_dev *hid, u8 * buffer)
{
	...
	if (is_e0) {
		handle_scancode(0xe0, !up);
	}

	handle_scancode(usar2scancode[ch], !up);
}
...
innovator_hid_open(struct input_dev *dev)
{
	...
	if (request_irq(INT_FPGA_ATN, (void *) innovator_hid_interrupt,
...
#if	defined(CONFIG_OMAP_INNOVATOR)	/* MVL-CEE */
	innovator_ps2_ldm_driver_register();
	innovator_ps2_ldm_device_register();
#endif				/* MVL-CEE */
...
{
#if	defined(CONFIG_OMAP_INNOVATOR)	/* MVL-CEE */
	innovator_ps2_ldm_device_unregister();
	innovator_ps2_ldm_driver_unregister();
#endif				/* MVL-CEE */
