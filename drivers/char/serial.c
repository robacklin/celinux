...
#define xscale_port(x)	((x) == PORT_XSCALE)
#else
#define xscale_port(x)	(0)
#endif

#ifdef CONFIG_ARCH_PXA
#define pxa_buggy_port(x) ({ \
	int cpu_ver; asm("mrc%? p15, 0, %0, c0, c0" : "=r" (cpu_ver)); \
	((x) == PORT_XSCALE && (cpu_ver & ~1) == 0x69052100); })
#else
...
static struct serial_uart_config uart_config[] = {
	...
	{ "XSCALE UART", 32, UART_CLEAR_FIFO | UART_USE_FIFO },
...
#endif
	case SERIAL_IO_MEM:
		if (xscale_port(info->state->type))
...
#if defined (CONFIG_OMAP_INNOVATOR) || defined(CONFIG_405LP) /* linux-pm */
...
{
        kgdb_irq = info->state->irq;
	kgdb_port = info->port;

        while(IRQ_ports[kgdb_irq]){
                shutdown(IRQ_ports[kgdb_irq]) ;
        }
}
#endif
...
#ifdef CONFIG_KGDB
	    (new_serial.irq == kgdb_irq) || (new_serial.port == kgdb_port) ||
#endif
