...
static int sEventStructSize[TRACE_EV_MAX + 1] =
{
  ...
  sizeof(trace_define_name)       /* TRACE_DEFINE_NAME */,
  sizeof(trace_dpm)               /* TRACE_DPM */
};
...
#ifdef LTT_MVISTA
static devfs_handle_t devfs_handle_tracer  = NULL;
static devfs_handle_t devfs_handle_traceru = NULL;
#endif
...
TraceEvent:
  ...
  if(ltt_test_bit(pmEventID, &sLogEventDetailsMask))
	...
	break;
      ...
      case TRACE_EV_DEFINE_NAME:
	lVarDataBeg = ((trace_define_name*) pmEventStruct)->event_name;
	lVarDataLen = ((trace_define_name*) pmEventStruct)->event_name_len;
	lDataSize += (uint16_t) lVarDataLen;
	break;

      case TRACE_EV_DPM:
	if(((trace_dpm*) pmEventStruct)->event_sub_id == TRACE_EV_DPM_OP)
	  {
	  lVarDataBeg  = ((trace_dpm*) pmEventStruct)->event_name;
	  lVarDataLen  = ((trace_dpm*) pmEventStruct)->event_data1;
	  lDataSize += (uint16_t) lVarDataLen;
	  }
	break;
		 ...
		 unsigned long pmArg)
{
  ...
  trace_buffer_start     lStartBufferEvent;     /* Start of the new buffer event */
  struct irqaction      *action;                /* An irq handler */
  int                    k;                     /* loop counter */
  ...
  switch(pmCmd)
	...
	return -EINVAL;
      ...
      ltt_set_bit(TRACE_EV_DEFINE_NAME, &sTracedEvents);
      ltt_set_bit(TRACE_EV_DEFINE_NAME, &sLogEventDetailsMask);
      ...
      lStartEvent.ExtraVersion  = TRACER_VERSION_EXTRA;
      ...
      for (k = 0; k < NR_IRQS; k++)
        {
        action = irq_desc[k].action;
	if ( !action || !action->handler )
	  continue;
	TRACE_DEFINE_NAME(TRACE_EV_DEFINE_NAME_IRQ, k, 0, action->name);
	for (action = action->next; action; action = action->next)
      ...
      for_each_task(p)
        {
	TRACE_DEFINE_NAME(TRACE_EV_DEFINE_NAME_PID, p->pid, p->p_pptr->pid, p->comm);
	}
...
#ifdef LTT_MVISTA
  sMajorNumber = LTT_TRACER_MAJOR;
  lError = devfs_register_chrdev(LTT_TRACER_MAJOR, TRACER_NAME, &sTracerFileOps);
  if (lError < 0) {
    printk(KERN_ERR "Tracer: Unable to register major %d, tracer disabled\n", LTT_TRACER_MAJOR);
    goto out;
  }
  devfs_handle_tracer = devfs_register (NULL, TRACER_NAME,
    DEVFS_FL_DEFAULT, LTT_TRACER_MAJOR, 0,
    S_IFCHR | S_IRUGO | S_IWUSR,
    &sTracerFileOps, NULL);
  devfs_handle_traceru = devfs_register (NULL, TRACER_NAME_U,
    DEVFS_FL_DEFAULT, LTT_TRACER_MAJOR, 1,
    S_IFCHR | S_IRUGO | S_IWUSR,
    &sTracerFileOps, NULL);
#else
...
#ifdef LTT_MVISTA
  devfs_unregister(devfs_handle_tracer);
  devfs_unregister(devfs_handle_traceru);
  devfs_unregister_chrdev(sMajorNumber, TRACER_NAME);
#else
