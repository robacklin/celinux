 ...
 */
static int __init iic_register_dev(void){
	struct ocp_dev* ocp;
	...
	if (!(ocp = ocp_alloc_dev(sizeof(struct ibm_iic_private)))){
		printk(KERN_CRIT "ibm-iic: failed to allocate device data\n");
		return -ENOMEM;
	}
	
	DBG2(": iic_register_dev, ocp = 0x%p\n", ocp);

	ocp->type = IIC;
	if ((ret = ocp_register(ocp)) < 0){
		printk(KERN_CRIT "ibm-iic: failed to register OCP device\n");
		goto fail3;
	}
	
	dev = (struct ibm_iic_private*)ocp->ocpdev;
	dev->idx = ocp->num;
...
/*
 * Cleanup all fully initialized IIC interfaces
 */
static void __exit iic_cleanup(void)
{
	struct ocp_dev* ocp;
	struct ibm_iic_private* dev;
	int i, max_iic = ocp_get_max(IIC);

	for (i = 0; i < max_iic; ++i) {
		ocp = ocp_get_dev(IIC, 0);
		
		DBG2(": iic_cleanup, %d, ocp = 0x%p\n", i, ocp);
		if (!ocp)
			break;
 ...
 * Loop through all configured IIC interfaces and register each one
 */
static int __init iic_init(void)
{
	int i, ret = 0, max_iic = ocp_get_max(IIC);
	printk(KERN_INFO "IBM IIC driver v" DRIVER_VERSION "\n");
	
	for (i = 0; i < max_iic && !ret; ++i)
		ret = iic_register_dev();
