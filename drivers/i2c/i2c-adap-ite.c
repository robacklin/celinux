...
static int iic_suspend(struct device *dev, u32 state, u32 level);
static int iic_resume(struct device *dev, u32 level);

static struct device_driver iic_driver_ldm = {
       name:      "i2c-adap-ite",
       devclass:  NULL,
       probe:     NULL,
       suspend:   iic_suspend,
...
static struct device iic_device_ldm = {
...
static void iic_ldm_driver_register(void)
{
   extern void ihb_driver_ldm_register(struct device_driver *driver);

   ihb_driver_ldm_register(&iic_driver_ldm);
}

static void iic_ldm_device_register(void)
{
   extern void ihb_device_ldm_register(struct device *device);

   ihb_device_ldm_register(&iic_device_ldm);
}

static void iic_ldm_driver_unregister(void)
{
   extern void ihb_driver_ldm_unregister(struct device_driver *driver);

   ihb_driver_ldm_unregister(&iic_driver_ldm);
}

static void iic_ldm_device_unregister(void)
{
   extern void ihb_device_ldm_unregister(struct device *device);

   ihb_device_ldm_unregister(&iic_device_ldm);
}
...
static int iic_suspend(struct device *dev, u32 state, u32 level)
{
  ...
  { 
     ...
     case SUSPEND_POWER_DOWN:
       ...
       printk(KERN_ERR "i2c suspend. \n");
       IT_IO_READ16(IT_PM_DSR, pmdsr);
       pmdsr |= IT_PM_DSR_I2CSB;
...
#ifdef CONFIG_MIPS_ITE8172 /* linux-pm */
	iic_ldm_device_register();
	iic_ldm_driver_register();
#endif /* linux-pm */
...
#ifdef CONFIG_MIPS_ITE8172 /* linux-pm */
        iic_ldm_driver_unregister();
        iic_ldm_device_unregister();
#endif               /* linux-pm */
}
