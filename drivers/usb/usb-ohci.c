...
#if defined(CONFIG_USB_OHCI_OMAP1510) \
 || defined(CONFIG_USB_OHCI_OMAP1510_MODULE)
extern void *omap_usb_pci_alloc_consistent(struct pci_dev *hwdev, 
					size_t size, dma_addr_t *handle);
extern void omap_usb_pci_free_consistent(struct pci_dev *hwdev, 
				size_t size, void *vaddr,
				dma_addr_t dma_handle);
extern dma_addr_t omap_usb_pci_map_single(struct pci_dev *hwdev, 
				void *ptr, size_t size, 
				int direction);
extern void omap_usb_pci_unmap_single(struct pci_dev *hwdev, 
				dma_addr_t dma_addr, size_t size, 
				int direction);
#define usb_ohci_pci_alloc_consistent omap_usb_pci_alloc_consistent
#define usb_ohci_pci_free_consistent omap_usb_pci_free_consistent
#define usb_ohci_pci_map_single omap_usb_pci_map_single
#define usb_ohci_pci_unmap_single omap_usb_pci_unmap_single
#else
#define usb_ohci_pci_alloc_consistent pci_alloc_consistent
#define usb_ohci_pci_free_consistent pci_free_consistent
#define usb_ohci_pci_map_single pci_map_single
#define usb_ohci_pci_unmap_single pci_unmap_single
#endif
...
#endif
#ifndef PCI_DEVICE_ID_ITE_OHCI
#define PCI_DEVICE_ID_ITE_OHCI        0x1234
#endif

static LIST_HEAD (ohci_hcd_list);
static spinlock_t usb_ed_lock = SPIN_LOCK_UNLOCKED;

#ifdef CONFIG_USB_OHCI_OMAP1510
extern void *omap_usb_pci_alloc_consistent(struct pci_dev *hwdev, 
					   size_t size, dma_addr_t *handle);
extern void omap_usb_pci_free_consistent(struct pci_dev *hwdev, 
					 size_t size, void *vaddr,
					 dma_addr_t dma_handle);
#define pci_alloc_consistent omap_usb_pci_alloc_consistent
#define pci_free_consistent omap_usb_pci_free_consistent
#endif
...
static int hc_ohci_suspend(struct device * dev, u32 state, u32 level);
static int hc_ohci_resume(struct device * dev, u32 level);
 
static struct device_driver hc_ohci_driver_ldm = {
       name:      	"USB_HC_ITE8172",
       devclass:  	NULL,
       probe:     	NULL,
       suspend:   	hc_ohci_suspend,
       resume:    	hc_ohci_resume,
       scale:	  	NULL,
       remove:    	NULL,
       constraints:	NULL,
};

static struct device hc_ohci_device_ldm = {
       name:		"ITE8172 USB ",
       bus_id:		"hc_ohci",
       driver: 		NULL,
...
static void hc_ohci_ldm_register(void)
{
	extern void pci_driver_ldm_register(struct device_driver *driver);
	extern void pci_device_ldm_register(struct device *device);
 
	pci_driver_ldm_register(&hc_ohci_driver_ldm);
	pci_device_ldm_register(&hc_ohci_device_ldm);
}

static void hc_ohci_ldm_unregister(void)
{
	...
	pci_driver_ldm_unregister(&hc_ohci_driver_ldm);
	pci_device_ldm_unregister(&hc_ohci_device_ldm);
}


static int ohci_pci_suspend (struct pci_dev *dev, u32 state);
static int ohci_pci_resume (struct pci_dev *dev);
...
static int hc_ohci_suspend(struct device * dev, u32 state, u32 level)
{
	...
	switch(level) { 
		...
		pcidevice = pci_find_device(PCI_VENDOR_ID_ITE,
		    PCI_DEVICE_ID_ITE_OHCI, NULL);
		ret = ohci_pci_suspend(pcidevice, state);
		break;
	}
...
static void urb_free_priv (struct ohci *hc, urb_priv_t * urb_priv)
{
	...
	if (last >= 0) {
		...
		if (usb_pipecontrol (td->urb->pipe)) {
			usb_ohci_pci_unmap_single (hc->ohci_dev, 
		...
		/* unmap data buffer */
		if (len && td->data_dma)
			usb_ohci_pci_unmap_single (hc->ohci_dev, td->data_dma, len, dir);
...
{ 
	...
	if (data_len) {
		data = usb_ohci_pci_map_single (ohci->ohci_dev,
	...
	switch (usb_pipetype (urb->pipe)) {
		...
		case PIPE_CONTROL:
			...
			td_fill (ohci, info,
				usb_ohci_pci_map_single (ohci->ohci_dev,
					urb->setup_packet, 8,
					PCI_DMA_TODEVICE),
				8, urb, cnt++); 
			if (data_len > 0) {  
...
static void hc_interrupt (int irq, void * __ohci, struct pt_regs * r)
{
	...
	{
		int readl_ints;
		readl_ints = readl(&regs->intrdisable);
...
// On unloaded USB, think 4k ~= 4-5msec
	...
	{
		int readl_ints;
		readl_ints = readl(&regs->intrenable);
		__asm__ __volatile__
		    ("sync\n\t");
	}
#endif
...
static ohci_t * __devinit hc_alloc_ohci (struct pci_dev *dev, void * mem_base)
{
	...
	ohci->hcca = usb_ohci_pci_alloc_consistent (dev, sizeof *ohci->hcca,
			&ohci->hcca_dma);
        if (!ohci->hcca) {
                kfree (ohci);
                return NULL;
...
#endif
		usb_ohci_pci_free_consistent (ohci->ohci_dev, 
			sizeof *ohci->hcca, ohci->hcca, ohci->hcca_dma);
		kfree (ohci);
		return NULL;
	}
#ifdef CONFIG_PCI
...
#ifndef CONFIG_ARCH_OMAP1510
#if !defined(CONFIG_SH_RTS7751R2D) && !defined(CONFIG_USB_OHCI_SH7760)
	iounmap (ohci->regs);
#endif
#endif

	usb_ohci_pci_free_consistent (ohci->ohci_dev, sizeof *ohci->hcca,
...
hc_add_ohci(struct pci_dev *dev, int irq, void *mem_base, unsigned long flags,
	    ohci_t **ohci_p, const char *name, const char *slot_name)
{
	char buf[8], *bufp = buf;
	ohci_t * ohci;
	int ret;
	...
	ohci = *ohci_p = hc_alloc_ohci (dev, mem_base);
	if (!ohci) {
		return -ENOMEM;
	}
...
#if !defined(CONFIG_USB_OHCI_SA1111) \
	&& !defined(CONFIG_USB_OHCI_SA1111_MODULE) \
	&& !defined(CONFIG_USB_OHCI_OMAP1510) \
	&& !defined(CONFIG_USB_OHCI_OMAP1510_MODULE) \
	&& !defined(CONFIG_USB_NON_PCI_OHCI) \
	&& !defined(CONFIG_USB_NON_PCI_OHCI_MODULE) \
	&& !defined(CONFIG_SH_RTS7751R2D) \
	&& !defined(CONFIG_USB_OHCI_SH7760)
