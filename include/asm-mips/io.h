...
extern void * __ioremap_readonly(phys_t offset, phys_t size,
				 unsigned long flags);
...
extern unsigned int tx_ioinl(unsigned int *addr);
...
#define inl(port)       (unsigned int)tx_ioinl((unsigned int *)(port))
#define outl_p(val,port)  tx_iooutl((val),(unsigned int *)(port))
#define inl_p(port) 	(unsigned int)tx_ioinl((unsigned int *)(port))
