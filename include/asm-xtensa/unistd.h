#ifndef __ASM_XTENSA_UNISTD_H
#define __ASM_XTENSA_UNISTD_H
...
/* syscalls 221 to 258 not defined in 2.4 kernel.  However,
 * need to keep syscall numbers of posix timers the same as
 * they are in 2.6
 */   
#define __NR_restart_syscall	258
...
/* The kernel uses this special-case system call to provide
   special-case functionality to user tasks without invoking the
   overhead of saving the entire context of the user task.  For
   maximum speed, the value must be 256 (or one of the nonlinear
   values defined in the Xtensa ISA for the "beqi" instruction, or
   equivalent). */
#define __NR_sysxtensa_quick    256
...
/* Note that we save and restore the a7 frame pointer.  Including
   a7 in the clobber list doesn't do what you'd expect. */
