...
#include <asm/mrshpc.h>
    ...
    defined(CONFIG_SH_MOBILE_SOLUTION_ENGINE) || \
...
static __inline__ int ide_default_irq_kzp(ide_ioreg_t base)
{
	switch (base) {
		case 0x01f0: return 11;
		case 0x0170: return 15;
		case 0x0320: return 14;
		default:
			return 0;
	}
}
...
static __inline__ int ide_default_irq(ide_ioreg_t base)
{
	...
	if (MACH_KZP) {
		return ide_default_irq_kzp(base);
	}
#endif
...
static __inline__ ide_ioreg_t ide_default_io_base_kzp(int index)
{
	switch (index) {
		case 0:	
			return 0x1f0;
...
static __inline__ ide_ioreg_t ide_default_io_base(int index)
{
	...
	if (MACH_KZP) {
		return ide_default_io_base_kzp(index);
	}
	switch (index) {
		case 0:	
			return 0x1f0;
...
#if defined(CONFIG_SH_KZP01)
		if(index==2)
			ide_init_hwif_ports(&hw, ide_default_io_base(index), 0x32e, NULL);
		else
#endif
