...
unsigned long s3c2440_get_rtc_time(void)
{
...
int s3c2440_set_rtc(void)
{
	...
	if (yeardiff < 0) {
		/* S3C2440 RTC forces that the year must be higher or 
...
static unsigned long s3c2440_gettimeoffset(void)
{
...
static void s3c2440_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	long flags;
...
static inline void setup_timer(void)
{
	...
	gettimeoffset = s3c2440_gettimeoffset;
	set_rtc = s3c2440_set_rtc;
	xtime.tv_sec = s3c2440_get_rtc_time();

	/* set timer interrupt */
	TCFG0 = (TCFG0_DZONE(0) | TCFG0_PRE1(15) | TCFG0_PRE0(0));
	TCFG1 = TCFG1 & ~(0xF<<16);

	pclk = s3c2440_get_pclk();
	TCNTB4 = ((pclk/1000)*625)/2000;

	TCON = (TCON_4_AUTO | TCON_4_UPDATE | COUNT_4_OFF);	
	timer_irq.handler = s3c2440_timer_interrupt;
	setup_arm_irq(IRQ_TIMER4, &timer_irq);
	TCON = (TCON_4_AUTO | COUNT_4_ON);
}

EXPORT_SYMBOL(s3c2440_get_rtc_time);
EXPORT_SYMBOL(s3c2440_set_rtc);
