...
pci_map_single(struct pci_dev *hwdev, void *ptr, size_t size, int direction)
{
	...
	extern dma_addr_t ixp425_pci_map_single(void *, size_t, int);
	return	ixp425_pci_map_single(ptr, size, direction);
#endif
...
{
#ifdef CONFIG_ARCH_IXP425
	extern void ixp425_pci_unmap_single(dma_addr_t, size_t, int);
	ixp425_pci_unmap_single(dma_addr, size, direction);
#endif
	if (dev_is_sa1111(hwdev))
...
pci_map_sg(struct pci_dev *hwdev, struct scatterlist *sg, int nents, int direction)
{
	...
	for (i = 0; i < nents; i++) {
		...
		if (sg[i].address) {
			...
			consistent_sync(page_address(sg[i].page) +
				        sg[i].offset, sg[i].length, direction);
...
{
#ifdef CONFIG_ARCH_IXP425
	extern void ixp425_pci_sync_single(dma_addr_t, size_t, int);
	ixp425_pci_sync_single(dma_handle, size, direction);
	return;
#endif
	if (dev_is_sa1111(hwdev)) {
