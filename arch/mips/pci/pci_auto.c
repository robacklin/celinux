...
/* Yes, this is a hack, but right now the pci_auto code does not
 * support devices that have special needs. This is needed in order
 * to establish a "common" memory map. 
 */
static u32 pciauto_cp_hbiu_base = 0x08000000;
static u32 pciauto_cp_hbiu_base_end = 0x08000000 + 0x08000000;
static u32 pciauto_cp_pcu_base = 0x1C000000;
static u32 pciauto_cp_pcu_base_end = 0x1C000000 + 0x04000000;
#endif
...
#ifdef CONFIG_ATI_XILLEON
			if ((PCI_SLOT(pci_devfn) == 0x14) && 
					(PCI_FUNC(pci_devfn) == 0) &&
					(bar == 0x10)) {
				lower_limit = &pciauto_cp_hbiu_base;
				upper_limit = &pciauto_cp_hbiu_base_end;
				DBG("        Mem");
			}
			else if ((PCI_SLOT(pci_devfn) == 0x14) && 
					(PCI_FUNC(pci_devfn) == 0) && 
					(bar == 0x1C)) {
				lower_limit = &pciauto_cp_pcu_base;
				upper_limit = &pciauto_cp_pcu_base_end;
				DBG("        Mem");
			}
			else
#endif
