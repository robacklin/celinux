 ...
 * prefer.
 */
int __noinstrument handle_IRQ_event(unsigned int irq, struct pt_regs * regs, struct irqaction * action)
{
	int status;
...
#ifdef CONFIG_VST
void
enable_jiffie_clock(void)
{
	/*
	 * On ARM they use the non-periodic timer to do VST
	 * On MIPS we use the common cpu0 clock so we don't
	 * disable it when we go into VST mode and we
	 * don't re-enable it here. This is a nop routine on MIPS
