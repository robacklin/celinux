 ...
 * <hack>
 *
 * Newer ARM kernels utilize a slightly more complex interrupt
 * handling scheme then older kernels that can lead to scheduling
 * in interrupt when pre-K is enabled and we have nested IRQ 
 * handlers. Basically what happens is this:
 *
 * - HW IRQ occurs
 * - vector_irq() is called
 * - do_asm_IRQ() is called and it grabs the irq_controller_lock
 * - do_IRQ()
 * - __do_irq() is called and it releases the irq_controller_lock 
 *   before calling the handler() function for that IRQ.
 *
 * In the case of multiple levels of interrupt controllers with
 * software demux, the handler for the IRQ simply reads some register
 * to determine the real IRQ number and then calls do_IRQ() again.
 * This in turn calls spin_unlock(&irq_controller_lock). What we're
 * seeing is an extra call to spin_unlock() which with pre-K enabled
 * translates to an extra decrease of the preempt_count thus allowing
 * pre-emption to occur. Due to this, we can hit the following 
 * assert in kernel/sched.c:schedule()
 *
 *	if (unlikely(in_interrupt()))
 *		BUG();
 *
 * To get around this issue, we just replace spin_lock and spin_unlock
 * with noop for this file. Since we only support uni-processor operation
 * and the preempt count is
 * this will have no real impact on the code.
 *
 * Kernel 2.6 solves this issue by replacing re-entrant calls to do_IRQ
 * with a new infrastructure to deal with multiple levels of interrupt
 * controllers.
 *
 * </hack>
 */
...
asmlinkage void asm_do_IRQ(int irq, struct pt_regs *regs)
{
	irq = fixup_irq(irq);
#ifdef CONFIG_VST
	if (update_vst_jiffies && irq != TIMER_IRQ) {
		enable_jiffie_clock();
	}
#endif
