...
int kgdb_enabled = 0;
int kgdb_enter = 0;		/* when set, wait for connection from gdb */
int kgdb_ttyS = 1;		/* which serial port to use, default ttyS1 */
int kgdb_baud = 9600;		/* baud rate to use */
...
{
	kgdb_ttyS = simple_strtoul(str,NULL,10);
	return 1;
}

static int __init kgdb_opt_gdbbaud(char *str)
{
	kgdb_baud = simple_strtoul(str,NULL,10);
	return 1;
}
