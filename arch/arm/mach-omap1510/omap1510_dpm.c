...
static void innovator_scale_divisors(struct dpm_regs *regs)
{
...
static void innovator_scale_dpll(struct dpm_regs *regs)
{
	volatile u16 *pll;
...
static void innovator_fscaler(struct dpm_regs *regs)
{
	...
	if(dpm_fscaler_flags & DPM_FSCALER_DPLL)
...
int
dpm_omap1510_init_opt(struct dpm_opt *opt)
{
...
int
dpm_omap1510_get_opt(struct dpm_opt *opt)
{
	struct dpm_md_opt *md_opt = &opt->md_opt;
...
{
	/*if ((read_ih(0, IRQ_ITR) & ~read_ih(0, IRQ_MIR)) ||
	    (read_ih(1, IRQ_ITR) & ~read_ih(1, IRQ_MIR)))
		return 1;
	else
		return 0;
		*/
...
dpm_omap_init(void)
{
	printk("OMAP1510 Innovator Dynamic Power Management\n");
	
	dpm_md.init		= NULL;
	dpm_md.init_opt		= dpm_omap1510_init_opt;
	dpm_md.set_opt		= dpm_default_set_opt;
	dpm_md.get_opt		= dpm_omap1510_get_opt;
	dpm_md.idle_set_parms	= NULL;
	dpm_md.cleanup		= dpm_omap_cleanup;

	dpm_omap1510_board_setup();
	dpm_bd.init();
