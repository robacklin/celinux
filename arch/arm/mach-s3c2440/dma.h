/*
 * linux/arch/arm/mach-s3c2440/dma.h
 *
 */
...
typedef struct {
	...
	int busy;		/* 1 if DMA is actually processing data */
	...
	int stop;		/* Is DMA channel 'stop request' ? 1 is Yes */
	int remain_size;	/* After stop, UnDMAed  size of current buffer */
		
				   
} s3c2440_dma_t;

s3c2440_dma_t dma_chan[MAX_S3C2440_DMA_CHANNELS];
