...
#ifdef CONFIG_VST
extern int vst_ms_to_jiffies(ctl_table *, int, struct file *,
    void *, size_t *);
...
static int vst_enable = 1;
static int vst_enable_min = 0;
static int vst_enable_max = 1;
unsigned long vst_threshold = 1*HZ;
static unsigned long vst_threshold_min = 1;
unsigned long vst_no_timers = 0;
unsigned long vst_short_timers = 0;
unsigned long vst_long_timers = 0;
/*
...
#define TMPBUFLEN 20
	...
	return proc_doulongvec_ms_jiffies_minmax(table, write, filp, buffer, lenp);
}
 ...
 * happen.  There are three possible outcomes (listed from highest
 ...
 * should occur.  The caller takes different actions based on the
 * return:
 *   NO_NEXT_TIMER: disable the tick interrupt but don't set an interval timer
 *   SHORT_NEXT_TIMER: leave the tick interrupt enabled
 *   RETURNED_NEXT_TIMER: disable the tick interrupt and set an interval timer
 */
int find_next_timer(unsigned long *when)
{
	...
	/* If VST is disabled, keep ticking. */
	if (!vst_enable)
		return SHORT_NEXT_TIMER;

	/* If there is something in the timer queue, keep ticking. */
	if (TQ_ACTIVE(tq_timer))
		return SHORT_NEXT_TIMER;

#ifdef CONFIG_CASCADING_TIMERS
	/*
	 * Each list in tv1 only contains timers that expire at the same
	 * time.  That means we don't need to shoot all the lists, just
	 * look for the first non-empty list and then look at its head.
	 */
	for (i = tv1.index; i < TVR_SIZE; i++) {
		entry = tv1.vec[i].next;
		if (entry != &tv1.vec[i]) {
			/* We found a non-empty list. Figure out the return */
			timer = list_entry(entry, struct timer_list, list);
			exp = timer->expires;
			if (time_after(threshold_jiffies, exp))
				return SHORT_NEXT_TIMER;
			*when = exp - jiffies;
...
#endif
	/*
	 * At this point, what we need to do for cascading timers and
	 * new timers is similar, but of course, different.
	 *
	 * For the cascading timers, we know that tv1 was empty.  We now
	 * need to shoot the lists in tv[2345].  Each list in those
	 * arrays may have timers that expire at different times so we
	 * need to find the first non-empty list and then find the
	 * soonest timer within that list.
	 *
	 * The new timers (as opposed to the cascading timers) aren't
	 * sorted, but rather hashed using the least significant bits of
	 * the expires field as the key.  That means we need to shoot
	 * *all* the lists to find the next timer.
	 *
	 * The bottom line is that we can use the same code for shooting
	 * the lists, but the looping constructs need to be different.
	 */
	next_jiffies = jiffies + MAX_TIMER_INTERVAL;
	rtn = NO_NEXT_TIMER;
#ifdef CONFIG_CASCADING_TIMERS
	for (i = 1; i< NOOF_TVECS; i++) {
		int j;
		for (j = tvecs[i]->index; j < TVN_SIZE; j++) {
			head = tvecs[i]->vec+j;
#else
	for (i=0; i<NEW_TVEC_SIZE; i++) {
			head = new_tvec+i;
			...
			list_for_each(entry, head) {
				timer = list_entry(entry, struct timer_list,
						   list);
				exp = timer->expires;
				if (time_after(threshold_jiffies, exp))
                                        return SHORT_NEXT_TIMER;
				if (time_after(next_jiffies, exp)) {
					next_jiffies = exp;
					rtn = RETURNED_NEXT_TIMER;
				}
			}
#ifdef CONFIG_CASCADING_TIMERS
			/* For cascading, we can quit if we've found a timer. */
			if (rtn == RETURNED_NEXT_TIMER) {
				*when = next_jiffies - jiffies;
				return rtn;
			}
		}
#endif
